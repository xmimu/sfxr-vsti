...
- **MIDI transposition**: A4 (note 69) maps to the "Start Frequency" knob's value; other notes transpose in semitones
- **Two trigger modes**: One-Shot (default, plays to completion) and Sustain (holds until note-off)
- **All 24 sfxr parameters** are DAW-automatable
- **Classic sfxr look**: beige/orange sliders, 7 preset generators + RANDOMIZE / MUTATE
- **On-screen MIDI keyboard** (full 88 keys, disabled keys greyed out, A4 highlighted) + real-time waveform oscilloscope
...
- **Virtual keyboard** (bottom): 88 keys, click/drag to play; A4 highlighted in orange; keys outside C2–C6 are greyed out and disabled

### Valid note range

The synth is rooted at A4 (note 69); other notes transpose by `2^(-(note-69)/12)`. The valid trigger range is **C2–C6 (MIDI 36–84)** — lower is essentially inaudible, higher tends to distort noise/explosion-type sounds. Notes outside this range are greyed out on screen, ignore mouse clicks, and DAW MIDI events are dropped.
...
| Start Frequency | 0–1 | start pitch (A4 maps to this value) |
...
- **Sample-rate independence**: envelope/repeat/arpeggio/vibrato time constants scaled by `sampleRate/44100`
- **MIDI transposition**: `fperiod *= 2^(-(note-69)/12)`, with A4 matching the original parameters
...
`tests/RenderTest.cpp` renders the engine offline to verify levels and transposition:
